...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        if (t == 0):
            self.V_prev = 0.5
        elif (self.V_prev < V_PREV_THRES):
            self.V_prev = V_PREV_THRES
        xdd = xdd_d + self.kpx*(x_d - x) + self.kdx*(xd_d - self.V_prev*np.cos(th))
        ydd = ydd_d + self.kpy*(y_d - y) + self.kdy*(yd_d - self.V_prev*np.sin(th))
        left = []
        right = []
        left.append([np.cos(th), -self.V_prev*np.sin(th)])
        right.append(xdd)
        left.append([np.sin(th), self.V_prev*np.cos(th)])
        right.append(ydd)
        a_omega = np.linalg.solve(np.array(left), np.array(right))
