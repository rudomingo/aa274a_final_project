...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        return (x[0] <= self.statespace_hi[0] and x[0] >= self.statespace_lo[0]
                and x[1] <= self.statespace_hi[1] and x[1] >= self.statespace_lo[1])
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        right = (x[0] + r, x[1])
        up_left = (x[0] - r, x[1] + r)
        up_right = (x[0] + r, x[1] + r)
        down_left = (x[0] - r, x[1] - r)
        down_right = (x[0] + r, x[1] - r)

        potential_neighbors = [up, down, left, right, up_left, up_right, down_left, down_right]
        for n in potential_neighbors:
            neighbor = self.snap_to_grid(n)
    ...
    def is_free(self, x):
        ...
        for obs in self.obstacles:
            ...
            for dim in range(len(x)):
                ...
                if x[dim] + .15 < obs[0][dim] or x[dim] - .15 > obs[1][dim]: 
                    inside = False
                    break
            if inside:
                return False
